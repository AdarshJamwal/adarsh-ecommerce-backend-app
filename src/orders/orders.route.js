...
router.post("/create-checkout-session", async(req, res)=>{
    ...
    try {
        ...
        const session = await stripe.checkout.sessions.create({
            ...
            mode:'payment',
            success_url: `https://adarsh-frontend-final.vercel.app/success?session_id={CHECKOUT_SESSION_ID}`,
            cancel_url: `https://adarsh-frontend-final.vercel.app/cancel`,
        })
...
router.get("/:email", async(req, res)=>{
    ...
    try {
        ...
        return res.status(500).send({message: "Failed to fetch orders by email"})
...
router.get("/order/:id", async(req, res)=>{
    try {
        ...
        return res.status(500).send({message: "Failed to fetch orders by user id"})
...
router.get("/", async(req, res)=>{
    try {
        ...
        return res.status(500).send({message: "Failed to fetch all orders"})
